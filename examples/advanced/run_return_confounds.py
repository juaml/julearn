...
from julearn import run_cross_validation, prepare_data
...
# `preprocess` method we return the transformed X, y and confounds up to
# the provided step (inclusive).
# This output consists of a tuple containing the transformed X, y and confound
X, y = prepare_data(X=feature_names, y='target', data=data,
                    confounds='sex')

X_deconfounded, _, confound = model.preprocess(
    X, y, until='remove_confound')
...
###############################################################################
# Above, you can see that we can set the `drop_confounds` argument to False.
...
scores, model = run_cross_validation(
    ...
    confounds='sex', model='linreg', problem_type='regression',
    preprocess_X=['remove_confound', 'pca'],
    model_params=dict(remove_confound__drop_confounds=False),
    return_estimator='final')
...
X_transformed, *_ = model.preprocess(X, y)
