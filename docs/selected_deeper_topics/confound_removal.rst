...
from such a confound removal model using the features [#3]_. 
In either case, it is very important that such confound regression models are only trained on the
...
Julearn implements cross-validation consistent confound regression for both of the
scenarios laid out above (i.e. either confound regression on the features or on
the target) allowing the user to implement complex machine learning pipelines with
...
First we create the features and the target, and based on this we create two artificial
confounds that we can use as an example:
...
Let's organise these data as a :class:`pandas.DataFrame`, which is the preferred
data format when using julearn, and also the most common data format for real-world
examples:
...
"confound_removal", and that the "X_type" called "confounds" should be used as confounds
in the confound regression. Note, that because we already specified apply_to="features"
...
.. code-block:: python
	...
	pipeline_creator = PipelineCreator(problem_type="regression", apply_to="features")
	pipeline_creator.add(target_pipeline, apply_to="target")
	pipeline_creator.add("linreg")
...
As you can see, applying confound regression in your machine learning workflow
in a cross-validated fashion is reasonably easy using julearn. If you are considering
whether or not to use confound regression, however, there are further important 
considerations:
...
learning pipeline is to account for the effects of the confounding variables on
the target. This can help to mitigate the potential bias introduced by the
...
necessary or appropriate, as it can lead to loss of valuable information in the data.
Additionally, confounding variables may sometimes be difficult to identify or
measure accurately, which can make confound regression challenging or ineffective.
In particular, controlling for some variables that are not confounds, but in fact
