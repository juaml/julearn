...
development version append *devN* where N is the distance (in commmits) to
the last release.
...
version "X.Y.Z". Additionaly, for every push to main, it will be published
...
2. Edit the file ``_version.py`` with the version of the next release. This
will be overwriten by `setuptools_scm`_, but it will make it consistent with
the version of the code in `julearn Github`_.
